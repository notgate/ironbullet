...
impl MitmCa {
    pub fn generate() -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        params
            .distinguished_name
            .push(rcgen::DnType::CommonName, "IronBullet Inspector CA");
        params
            .distinguished_name
            .push(rcgen::DnType::OrganizationName, "IronBullet");
...
async fn handle_connect(
    ...
    let (cert_pem, key_pem) = match ca.sign_for_host(&hostname) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("[mitm] cert error for {hostname}: {e}");
            return;
        }
    };
    ...
    let acceptor = match make_tls_acceptor(&cert_pem, &key_pem) {
        Ok(a) => a,
        Err(e) => {
            eprintln!("[mitm] acceptor error: {e}");
            return;
        }
    };
    let tls_stream = match acceptor.accept(client).await {
        Ok(s) => s,
        Err(e) => {
            eprintln!("[mitm] TLS accept error for {hostname}: {e}");
            return;
        }
    };
    ...
    loop {
        ...
        if !req_body.is_empty() {
            upstream_rw.write_all(&req_body).await.ok();
        }
        upstream_rw.flush().await.ok();

        // Read upstream response headers
        let (status, status_text, resp_raw_headers, resp_headers) =
            match read_response_head(&mut upstream_rw).await {
                Some(v) => v,
                None => break,
            };
        ...
        let client_writer = client_reader.get_mut();
        client_writer
            .write_all(resp_raw_headers.as_bytes())
            .await
            .ok();
        ...
        if conn.to_lowercase().contains("close") || (!has_cl && !is_chunked) {
            break;
        }
    }
}
...
async fn handle_http_plain(
    ...
    let mut upstream = match TcpStream::connect(format!("{}:{}", upstream_host, upstream_port))
        .await
    {
        Ok(s) => s,
        Err(e) => {
    ...
    if !req_body.is_empty() {
        upstream.write_all(&req_body).await.ok();
    }

    let mut upstream_reader = BufReader::new(upstream);
    let (status, status_text, resp_raw, resp_headers) =
        match read_response_head(&mut upstream_reader).await {
            Some(v) => v,
            None => return,
        };
    let has_cl = resp_headers.contains_key("content-length");
...
async fn read_chunked<R: AsyncBufReadExt + AsyncReadExt + Unpin>(
    reader: &mut R,
    max: usize,
) -> Vec<u8> {
    let mut out = Vec::new();
...
fn make_tls_acceptor(
    ...
    let key_der = rustls_pemfile::private_key(&mut key_pem.as_bytes())?.ok_or("no private key")?;
...
fn infer_resource_type(
    ...
    let ct = headers
        ...
        .map(|s| s.as_str())
        .unwrap_or("");
    if ct.contains("application/json") || accept.contains("application/json") {
        return "fetch";
    }
    ...
    if path.ends_with(".css") || path.contains(".css?") {
        return "stylesheet";
    }
