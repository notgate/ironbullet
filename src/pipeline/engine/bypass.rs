...
impl ExecutionContext {
    ...
    pub(super) async fn execute_webhook(
        ...
        let _resp = resp_rx.await.map_err(|_| {
            crate::error::AppError::Sidecar("Webhook response channel closed".into())
        })?;
        Ok(())
    }
    ...
    pub(super) fn execute_random_user_agent(
        &mut self,
        settings: &RandomUserAgentSettings,
    ) -> crate::error::Result<()> {
        use rand::seq::SliceRandom;

        if settings.match_tls {
            // Use TLS profiles for matched UA + JA3 + HTTP/2 fingerprint
            let filtered: Vec<_> = TLS_PROFILES
                .iter()
                .filter(|p| {
                    let browser_ok = settings.browser_filter.is_empty()
                        || settings
                            .browser_filter
                            .iter()
                            .any(|f| f.eq_ignore_ascii_case(p.browser));
                    let platform_ok = settings.platform_filter.is_empty()
                        || settings
                            .platform_filter
                            .iter()
                            .any(|f| f.eq_ignore_ascii_case(p.platform));
        ...
        let ua = match settings.mode {
            ...
            UserAgentMode::Random => {
                let filtered: Vec<&(&str, &str, &str)> = BUILTIN_USER_AGENTS
                    .iter()
                    .filter(|(_, browser, platform)| {
                        let browser_ok = settings.browser_filter.is_empty()
                            || settings
                                .browser_filter
                                .iter()
                                .any(|f| f.eq_ignore_ascii_case(browser));
                        let platform_ok = settings.platform_filter.is_empty()
                            || settings
                                .platform_filter
                                .iter()
                                .any(|f| f.eq_ignore_ascii_case(platform));
                        browser_ok && platform_ok
                    })
                    .collect();
                filtered
        ...
        self.variables
            .set_user(&settings.output_var, ua, settings.capture);
        Ok(())
    }

    // ── OCR Captcha ──

    pub(super) async fn execute_ocr_captcha(
        &mut self,
        settings: &OcrCaptchaSettings,
    ) -> crate::error::Result<()> {
        ...
        let image_bytes = base64::engine::general_purpose::STANDARD
            ...
            .map_err(|e| {
                crate::error::AppError::Pipeline(format!("OCR base64 decode error: {}", e))
            })?;

        // Write to temp file
        let temp_path =
            std::env::temp_dir().join(format!("ironbullet_ocr_{}.png", uuid::Uuid::new_v4()));
        std::fs::write(&temp_path, &image_bytes).map_err(|e| {
            crate::error::AppError::Pipeline(format!("OCR temp file write error: {}", e))
        })?;
        ...
        let image = rusty_tesseract::Image::from_path(&temp_path).map_err(|e| {
            crate::error::AppError::Pipeline(format!("OCR image load error: {}", e))
        })?;

        let result = rusty_tesseract::image_to_string(&image, &args).map_err(|e| {
            crate::error::AppError::Pipeline(format!("OCR recognition error: {}", e))
        })?;
    ...
    pub(super) async fn execute_recaptcha_invisible(
        ...
        let anchor_req = SidecarRequest {
            ...
            headers: Some(vec![vec!["User-Agent".into(), user_agent.clone()]]),
        ...
        let (resp_tx, resp_rx) = tokio::sync::oneshot::channel();
        sidecar_tx
            .send((anchor_req, resp_tx))
            .await
            .map_err(|_| crate::error::AppError::Sidecar("Failed to send anchor request".into()))?;
        let anchor_resp = resp_rx.await.map_err(|_| {
            crate::error::AppError::Sidecar("Anchor response channel closed".into())
        })?;
            ...
            if let Some(start) = body.find(marker) {
                ...
                if let Some(end) = body[after..].find('"') {
                    ...
                    return Err(crate::error::AppError::Pipeline(
                        "Failed to parse recaptcha-token end".into(),
                    ));
                }
            } else {
                return Err(crate::error::AppError::Pipeline(
                    "recaptcha-token not found in anchor response".into(),
                ));
            }
        };
        ...
        let (resp_tx, resp_rx) = tokio::sync::oneshot::channel();
        sidecar_tx
            .send((reload_req, resp_tx))
            .await
            .map_err(|_| crate::error::AppError::Sidecar("Failed to send reload request".into()))?;
        let reload_resp = resp_rx.await.map_err(|_| {
            crate::error::AppError::Sidecar("Reload response channel closed".into())
        })?;
        ...
        let rresp = {
            ...
            if let Some(start) = body.find(marker) {
                ...
                if let Some(end) = body[after..].find('"') {
                    ...
                    return Err(crate::error::AppError::Pipeline(
                        "Failed to parse rresp end".into(),
                    ));
                }
            } else {
                return Err(crate::error::AppError::Pipeline(
                    "rresp not found in reload response".into(),
                ));
            }
        };

        self.variables
            .set_user(&settings.output_var, rresp, settings.capture);
        Ok(())
    }

    // ── XACF Sensor ──

    pub(super) fn execute_xacf_sensor(
        &mut self,
        settings: &XacfSensorSettings,
    ) -> crate::error::Result<()> {
        ...
        self.variables
            .set_user(&settings.output_var, sensor, settings.capture);
        Ok(())
    }

    // ── Random Data ──

    pub(super) fn execute_random_data(
        &mut self,
        settings: &RandomDataSettings,
    ) -> crate::error::Result<()> {
        let value = match settings.data_type {
            RandomDataType::String => random_data::random_string(
                settings.string_length as usize,
                &settings.string_charset,
                &settings.custom_chars,
            ),
            RandomDataType::Uuid => random_data::random_uuid(),
            RandomDataType::Number => random_data::random_number(
                settings.number_min,
                settings.number_max,
                settings.number_decimal,
            ),
            ...
            RandomDataType::Date => random_data::random_date(
    ...
    pub(super) fn execute_datadome_sensor(
        &mut self,
        settings: &DataDomeSensorSettings,
    ) -> crate::error::Result<()> {
        ...
        let custom_wasm = if settings.custom_wasm_b64.is_empty() {
            ...
            base64::engine::general_purpose::STANDARD
                .decode(settings.custom_wasm_b64.as_bytes())
                .ok()
        };
        let sensor =
            datadome::generate_datadome_sensor(&site_url, &cookie, &ua, custom_wasm.as_deref())?;
        self.variables
            .set_user(&settings.output_var, sensor, settings.capture);
        Ok(())
    }
    ...
    pub(super) fn execute_plugin_block(
        &mut self,
        settings: &PluginBlockSettings,
    ) -> crate::error::Result<()> {
        let pm = self.plugin_manager.as_ref().ok_or_else(|| {
            crate::error::AppError::Pipeline("No plugin manager available".into())
        })?;
        ...
        let (_, updated_vars, log) = pm
            .execute_block(&settings.plugin_block_type, &settings_json, &vars_snapshot)
        ...
        if !log.is_empty()
            && self
                .variables
                .get(&settings.output_var)
                .unwrap_or_default()
                .is_empty()
        {
            self.variables
                .set_user(&settings.output_var, log, settings.capture);
        }
        Ok(())
    }
            ...
            if let Ok(resp) = resp_rx.await {
                ...
                if status == "ready" {
                    let token = json["solution"]["gRecaptchaResponse"]
                        ...
                        .to_string();
                    self.variables
                        .set_user(&settings.output_var, token, settings.capture);
                    return Ok(());
                }
            }
    ...
    pub(super) async fn execute_cloudflare_bypass(
        ...
        let user_agent = json["solution"]["userAgent"]
            .as_str()
            .unwrap_or("")
            .to_string();
        ...
        self.variables
            .set_user(&settings.output_var, cookies, settings.capture);
        Ok(())
    }
    ...
    pub(super) fn execute_akamai_v3_sensor(
        &mut self,
        settings: &AkamaiV3SensorSettings,
    ) -> crate::error::Result<()> {
        ...
        match settings.mode {
            AkamaiV3Mode::Encrypt => {
                ...
                let sensor_data =
                    format!("3;0;1;0;{};{};141659;{}", cookie_hash, file_hash, encrypted);
                self.variables
                    .set_user(&settings.output_var, sensor_data, settings.capture);
            }
            AkamaiV3Mode::Decrypt => {
                ...
                self.variables
                    .set_user(&settings.output_var, decrypted, settings.capture);
            }
    ...
    pub(super) async fn execute_laravel_csrf(
        ...
        let sel = scraper::Selector::parse(&selector).map_err(|e| {
            crate::error::AppError::Pipeline(format!("Invalid CSRF selector: {:?}", e))
        })?;
        ...
        if !settings.cookie_name.is_empty() {
            if let Some(ref cookies) = resp.cookies {
                if let Some(cookie_val) = cookies.get(&settings.cookie_name) {
                    self.variables.set_user(
                        &format!("{}_COOKIE", settings.output_var),
                        cookie_val.clone(),
                        false,
                    );
                }
