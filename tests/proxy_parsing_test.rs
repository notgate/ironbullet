...
fn test_socks5_port_detection() {
    ...
    assert!(
        ...
        "Port 1080 should be detected as SOCKS5, got: {}",
        proxy
    );
}
...
fn test_explicit_scheme_takes_precedence() {
    ...
    assert!(
        ...
        "Explicit http:// should be preserved, got: {}",
        proxy
    );
}
...
fn test_tor_port_detection() {
    ...
    assert!(
        proxy.starts_with("socks5://"),
        "Port 9050 (Tor) should be detected as SOCKS5, got: {}",
        proxy
    );
}
...
fn test_http_default_for_unknown_ports() {
    ...
    assert!(
        proxy.starts_with("http://"),
        "Port 8080 should default to HTTP, got: {}",
        proxy
    );
}
...
fn test_default_type_override() {
    ...
    assert!(
        proxy.starts_with("socks5://"),
        "default_type should override port detection, got: {}",
        proxy
    );
}
...
fn test_authenticated_socks5() {
    ...
    assert!(
        proxy.starts_with("socks5://"),
        "Authenticated SOCKS5 should be detected, got: {}",
        proxy
    );
...
fn test_colon_separated_auth_format() {
    ...
    assert!(
        proxy.starts_with("socks5://"),
        "Colon format SOCKS5 should be detected, got: {}",
        proxy
    );
...
fn test_shadowsocks_detection() {
    ...
    assert!(
        proxy.starts_with("socks5://127.0.0.1:"),
        "Shadowsocks should resolve to local SOCKS5 tunnel, got: {}",
        proxy
    );
}
...
fn test_multiple_proxy_types_mixed() {
    ...
    assert!(
        proxies.iter().any(|p| p.starts_with("http://proxy1.com")),
        "Should have explicit HTTP"
    );
    assert!(
        proxies
            .iter()
            .any(|p| p.starts_with("socks5://127.0.0.1:1080")),
        "Should have auto-detected SOCKS5"
    );
    assert!(
        proxies
            .iter()
            .any(|p| p.starts_with("socks5://proxy2.com:1081")),
        "Should have explicit SOCKS5"
    );
    assert!(
        proxies
            .iter()
            .any(|p| p.starts_with("http://192.168.1.1:3128")),
        "Should have auto HTTP"
    );
}
